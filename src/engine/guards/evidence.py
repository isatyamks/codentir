...
class EvidenceThreshold:
    ...
    def __init__(self, min_confidence: float = None, min_chunks: int = 1):
    ...
    def requires_clarification(self, retrieval_results: List[Dict[str, Any]]) -> bool:

        check = self.check_evidence(retrieval_results)
        return not check["sufficient"]

    def get_clarification_prompt(self, query: str, check_result: Dict[str, Any]) -> str:
